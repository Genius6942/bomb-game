(function (global, factory) {
  ...
  class Joystick {
    constructor(options = {}) {
      ...
      this.options = { outerRadius: 100, innerRadius: 75 / 2, ...options };
      this.config();
      ...
      this.dom.firstElementChild.innerHTML = `
		...
		`;
      this.grabber = { x: 0, y: 0, grabbing: false };
      this._handlePosition = { x: 0, y: 0 };
      ...
      document.addEventListener("mouseup", this.mouseHandler.bind(this), {
        passive: false,
      });
      this.dom.firstElementChild.firstElementChild.addEventListener(
        "touchstart",
      ...
      document.addEventListener("touchend", this.touchHandler.bind(this), {
        passive: false,
      });
      this.updateStyle();
    }
    ...
    mouseHandler(e) {
      if (e.type === "mousedown") {
        ...
        this.events.start.forEach((callback) =>
          callback(this.angle, this.grabber.grabbing)
        );
        ...
        if (this.grabber.grabbing) {
          ...
          this.handlePosition = { x: x, y: y };
          this.calculateAngle();
          this.events.move.forEach((callback) =>
            callback(this.angle, this.grabber.grabbing)
          );
        }
      }
    ...
    touchHandler(e) {
      if (e.type === "touchstart") {
        ...
        this.grabber.x = [...e.touches].find(
          ({ identifier }) => identifier === this.touchIdentifier
        ).clientX;
        this.grabber.y = [...e.touches].find(
          ({ identifier }) => identifier === this.touchIdentifier
        ).clientY;
        this.events.start.forEach((callback) =>
          callback(this.angle, this.grabber.grabbing)
        ...
        if (this.grabber.grabbing) {
          e.preventDefault();
          let x =
            [...e.touches].find(({ identifier }) => identifier === this.touchIdentifier)
              .clientX - this.grabber.x;
          let y =
            [...e.touches].find(({ identifier }) => identifier === this.touchIdentifier)
              .clientY - this.grabber.y;
          if (Math.sqrt(x * x + y * y) > this.options.outerRadius) {
            const ratio = this.options.outerRadius / Math.sqrt(x * x + y * y);
            x *= ratio;
            y *= ratio;
